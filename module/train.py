...
class Trainer:
    def __init__(self, config, model, train_dataloader, valid_dataloader):
        ...
        self.ckpt = config.ckpt if config.mode=='train' else config.pt_ckpt
        self.record_path = self.ckpt.replace('model.pt', 'report.json')
        self.encoder_train = config.mode == 'pretrain' and config.pt_obj == 'masked'
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            if best_loss > valid_epoch_loss:
                ...
                if self.encoder_train:
                    model_state_dict = {k: v for k, v in self.model.state_dict().items() if 'pooler' not in k}
                else:
